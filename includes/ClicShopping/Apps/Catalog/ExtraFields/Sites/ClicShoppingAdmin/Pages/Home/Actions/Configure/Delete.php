<?php
  ...
  class Delete extends \ClicShopping\OM\PagesActionsAbstract
  {
    ...
    public function execute()
    {
      ...
      static::removeMenu();
      static::removeProductsExtraFieldsDb();

      Cache::clear('menu-administrator');

      $CLICSHOPPING_MessageStack->add($CLICSHOPPING_ExtraFields->getDef('alert_module_uninstall_success'), 'success');
    ...
    private static function removeMenu()
    {
      ...
      if ($Qcheck->fetch()) {
        ...
        $QMenuId->bindValue(':app_code', 'app_catalog_extra_fields');
        $QMenuId->execute();
        ...
        $CLICSHOPPING_Db->delete('administrator_menu', ['app_code' => 'app_catalog_extra_fields']);
      }
    }

    private static function removeProductsExtraFieldsDb()
    {
      $CLICSHOPPING_Db = Registry::get('Db');
      ...
      if ($Qcheck->fetch() !== false) {
        $Qdelete = $CLICSHOPPING_Db->prepare('delete from :table_products_extra_fields');
        $Qdelete->execute();
      }
      ...
      if ($Qcheck->fetch() !== false) {
        $Qdelete = $CLICSHOPPING_Db->prepare('delete from :table_products_to_products_extra_fields');
        $Qdelete->execute();
      }
    }
  }